...
##### To be Updated #####
student_id = "2023AB050842"  # Example ID, replace with your actual student ID
#########################

student_id = ''.join([i for i in student_id if i.isdigit()])
# random.seed(int(student_id)) # (124, 0.01), -3.13
random.seed(student_id) # (122, 0.01), -3.13
...
for i in range(X.shape[0]):
    for j in range(X.shape[1]):
        ...
        # print(f"Z[{i}, {j}]: {Z[i][j]}")
        # print(f"Z[{i}, {j}][0][0]: {Z[i][j]}")
        #Z[i][j] = C(np.array([X[i][j],Y[i][j]]).reshape(2,1))[0][0]
